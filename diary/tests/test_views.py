...
class LoggedInTestCase(TestCase):
  """各テストクラスで共通の事前準備処理をオーバーライドした独自てTestCaseクラス"""

  def setUp(self):
    """テストメソッド実行前の事前設定"""

    # テストユーザーのパスワード
    self.password = 't4169n5175'

    # 各インスタンスメソッドを使うテスト用ユーザーを生成しインスタンス変数に格納しておく
    self.test_user = get_user_model().objects.create_user(
      username='user_test_man',
      email="pass_mail39-sns@yahoo.co.jp",
...
class TestDiaryCreateView(LoggedInTestCase):
  ...
  def test_create_diary_success(self):
    ...
    # 日記データがDBに登録されたかを検証
    self.assertEqual(Diary.objects.filter(title='テストで書くタイトル').count(), 1)
